...
def WPS_chrom(chrom='chr1', step=10):
    ...
    for read in bamfile.fetch(chrom, multiple_iterators=True):
        chrom_reads.add_interval(
            Interval(read.reference_start, read.reference_end))
    ...
    for ra, rb in chrom_regions.values:
        ...
        for pos in range(ra, rb + 1, step):
            ...
            for read in chrom_reads.find(wa, wb):
                if (read.start > wa) or (read.end < wb): endCount += 1
                else: comCount += 1
...
all_chrom_WPS = Parallel(n_jobs=-1, verbose=1, backend='multiprocessing')(
    delayed(WPS_chrom)(chrom) for chrom in tqdm(chrom_list))

pickle.dump(
    np.hstack(all_chrom_WPS) * (1000000 / bamfile.count()),
    open(options.outfile, 'wb'))